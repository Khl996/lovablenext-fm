-- Update work_order_reject to allow technician rejection from 'assigned' status
...
BEGIN
  ...
  IF _rejection_stage = 'technician' THEN
    -- Technician rejects from assigned OR in_progress -> returns to assigned (for supervisor to reassign)
    -- Status stays assigned if already assigned, or reverts to assigned from in_progress
    _new_status := 'assigned';
    
    -- Validate current status - technician can reject from assigned or in_progress
    IF _work_order.status NOT IN ('assigned', 'in_progress') THEN
      RAISE EXCEPTION 'Technician can only reject from assigned or in_progress status';
    END IF;
